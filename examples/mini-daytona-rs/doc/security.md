...
mini-daytona-rs 当前采用“双 profile”沙箱设计：
...
每个沙箱拥有独立 network namespace，通过 veth pair 接入 `daytona0` bridge，并使用 iptables NAT 访问外网。
...
```bash
cd examples/mini-daytona-rs && node test/run_e2e.js --client
```
...
当前 mini-daytona-rs 的安全模型已经从“单一宽松沙箱”演进到“Build/Runtime 双 profile 沙箱”：
