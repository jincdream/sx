...
mini-daytona-rs 通过 Linux Network Namespace + veth pair + 网桥 + iptables NAT 为每个沙箱提供独立的网络环境，同时支持沙箱访问外部网络。
...
ip addr add 10.200.0.1/24 dev daytona0
ip link set daytona0 up
...
- `daytona0` 网桥地址固定为 `10.200.0.1/24`
...
# host 端加入网桥
ip link set veth-h-{idx} master daytona0
