...
| `get_data_dir()` | 返回 `~/.mini-daytona/` |
| `get_snapshots_dir()` | 返回 `~/.mini-daytona/snapshots/` |
| `get_bases_dir()` | 返回 `~/.mini-daytona/bases/` |
| `get_cache_dir()` | 返回 `~/.mini-daytona/cache/` |
| `get_sandboxes_dir()` | 返回 `~/.mini-daytona/sandboxes/` |
...
- **路径**：`~/.mini-daytona/metadata/state.json`
- **策略**：
...
1. **创建网桥** (`ensure_bridge`)：
   - `ip link add daytona0 type bridge`
   - 分配 IP `10.200.0.1/24`
...
```text
mini-daytona-rs [OPTIONS] <COMMAND>
