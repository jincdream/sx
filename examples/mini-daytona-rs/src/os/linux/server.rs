...
pub fn get_sandbox_metrics(sandbox: &SandboxMetadata) -> SandboxRuntimeStatsResponse {
    ...
    let cgroup_path = std::path::PathBuf::from("/sys/fs/cgroup/mini-daytona").join(&sandbox.id);
...
pub fn read_oom_kill_count(sandbox_id: &str) -> Option<u64> {
    let cgroup_path = std::path::PathBuf::from("/sys/fs/cgroup/mini-daytona").join(sandbox_id);
...
pub fn exec_sandbox(
    ...
    if let Some(pid) = sandbox.pid {
        ...
        let cgroup_procs = format!("/sys/fs/cgroup/mini-daytona/{}/cgroup.procs", sandbox.id);
