...
if [ ! -f "$TARGET_DIR/moulin" ]; then
    err "导出完成，但未找到产物: $TARGET_DIR/moulin"
    exit 1
fi

chmod +x "$TARGET_DIR/moulin"
ok "产物导出完成: $TARGET_DIR/moulin"
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "  导出产物:  ${CYAN}$TARGET_DIR/moulin${NC}"
echo -e "  目标平台:  ${CYAN}$DOCKER_PLATFORM${NC}"
echo -e ""
echo -e "  Linux 运行: BINARY_PATH=$TARGET_DIR/moulin SKIP_BUILD=1 bash start-local.sh"
echo -e "${GREEN}========================================${NC}"