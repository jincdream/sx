...
fn setup_cgroups(sandbox_id: &str, pid: i32, limits: &ResourceLimits) -> anyhow::Result<()> {
    ...
    let cg_base = "/sys/fs/cgroup/moulin";
    ...
    //   └─ /sys/fs/cgroup/moulin/cgroup.subtree_control → +memory +pids +cpu
    //       └─ /sys/fs/cgroup/moulin/<sandbox-id>/memory.max  ← writable
    ...
    // Step 2: Ensure moulin cgroup directory exists
    if !fs::metadata(cg_base).is_ok() {
        fs::create_dir_all(cg_base)?;
    }

    // Step 3: Enable controllers at the moulin level
    if let Err(e) = fs::write(
        format!("{}/cgroup.subtree_control", cg_base),
        "+memory +pids +cpu",
    ) {
        warn!(
            "Failed to enable moulin cgroup subtree controllers: {}",
            e
        );
    }
...
fn cleanup_cgroups(sandbox_id: &str) -> anyhow::Result<()> {
    let cg_path = format!("/sys/fs/cgroup/moulin/{}", sandbox_id);
