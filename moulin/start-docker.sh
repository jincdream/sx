...
# ---- 可配置参数 (环境变量覆盖) ----
IMAGE_NAME="${IMAGE_NAME:-moulin}"
CONTAINER_NAME="${CONTAINER_NAME:-moulin-server}"
VOLUME_NAME="${VOLUME_NAME:-moulin-data}"
PORT="${PORT:-3000}"
...
docker run -d \
    ...
    --privileged \
    -p "${PORT}:3000" \
    -e HOME=/var/run/moulin_home \
    -e TMPDIR=/var/run/moulin_home/tmp \
    -v "${VOLUME_NAME}:/var/run/moulin_home" \
    -v "${SCRIPT_DIR}/images:/var/run/moulin_home/images" \
    "$IMAGE_NAME" \
    sh -c 'mkdir -p /var/run/moulin_home/tmp && ./target/release/moulin server' \
    >/dev/null
