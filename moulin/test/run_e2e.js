...
const SERVER_BINARY = process.env.BINARY_PATH || './target/release/moulin';
...
if (!CLIENT_ONLY) {
  let baseHome = process.env.moulin_HOME || '/var/run/moulin_home';
  process.env.HOME = baseHome;
  process.env.TMPDIR = path.join(baseHome, 'tmp');
  try {
    fs.mkdirSync(process.env.TMPDIR, { recursive: true });
  } catch (err) {
    if (err.code === 'EACCES' && !process.env.moulin_HOME) {
      console.warn(`\n[WARN] Permission denied creating ${process.env.TMPDIR}`);
      console.warn(`[WARN] Falling back to /tmp/moulin_home for local E2E test`);
      console.warn(`[WARN] (If testing a Docker server, remember to pass '--client')\n`);
      baseHome = '/tmp/moulin_home';
      process.env.HOME = baseHome;
      process.env.moulin_HOME = baseHome;
      process.env.TMPDIR = path.join(baseHome, 'tmp');
      fs.mkdirSync(process.env.TMPDIR, { recursive: true });
    } else {
      throw err;
...
async function runTests() {
  ...
  const getBuildPath = (subPath) => CLIENT_ONLY ? `/var/run/moulin_home/${subPath}` : path.join(projectRoot, subPath);
  ...
  if (shouldRunTest('api') || shouldRunTest('nginx')) {
    ...
    if (serverCapabilities.os === 'macos') {
      ...
      assert(pageContent.includes('Hello Moulin'), 'Nginx content mismatch when accessed from an external container');
      ...
      try {
        ...
        assert(hostPageContent.includes('Hello Moulin'), 'Host: Nginx content mismatch');
      } catch (err) {
...
`;
    const daWriteRes = await timedRequest('DataAnalysis', 'file_write', 'POST', `/sandbox/${daSandboxId}/file`, {
      path: '/home/moulin/workspace/da_script.py',
    ...
    if (!canRunImageExec) {
      ...
      const daExecRes = await timedRequest('DataAnalysis', 'exec', 'POST', `/sandbox/${daSandboxId}/exec`, {
        cmd: ['python3', '/home/moulin/workspace/da_script.py']
      });
  ...
  if (shouldRunTest('upload') || shouldRunTest('excel')) {
    ...
    const uploadRes = await timedRequest('Upload', 'upload', 'POST', `/sandbox/${ulSandboxId}/upload`, {
      path: '/home/moulin/workspace/dataset.xlsx',
      data: xlsxBase64
    });
...
try:
    df = pd.read_excel('/home/moulin/workspace/dataset.xlsx', engine='openpyxl')
...
`;
    const scriptWriteRes = await timedRequest('Upload', 'file_write', 'POST', `/sandbox/${ulSandboxId}/file`, {
      path: '/home/moulin/workspace/process_xlsx.py',
      content: xlsxScript
    });
    ...
    if (!canRunImageExec) {
      ...
      const xlsxExecRes = await timedRequest('Upload', 'exec', 'POST', `/sandbox/${ulSandboxId}/exec`, {
        cmd: ['python3', '/home/moulin/workspace/process_xlsx.py']
      });
    ...
    const downloadRes = await timedRequest('Upload', 'download', 'GET', `/sandbox/${ulSandboxId}/download?path=/home/moulin/workspace/dataset.xlsx`);
  ...
  if (shouldRunTest('puppeteer')) {
    ...
    if (!canRunImageExec) {
      ...
      const pbWriteRes = await timedRequest('Puppeteer', 'file_write', 'POST', `/sandbox/${pbSandboxId}/file`, {
        path: '/home/moulin/workspace/test-puppeteer.js',
      ...
      if (!canRunImageExec) {
        ...
        const pbExecRes = await timedRequest('Puppeteer', 'exec', 'POST', `/sandbox/${pbSandboxId}/exec`, {
          cmd: ['node', '/home/moulin/workspace/test-puppeteer.js']
        });
  ...
  if (shouldRunTest('nextjs') || shouldRunTest('next')) {
    ...
    if (!canRunImageExec) {
      ...
      const nextCreateRes = await timedRequest('Next.js', 'exec', 'POST', `/sandbox/${nextSandboxId}/exec`, {
        cmd: ['create-next-app', '/home/moulin/workspace/my-app', '--yes', '--javascript', '--no-tailwind', '--no-eslint', '--app', '--no-src-dir', '--import-alias', '@/*', '--use-npm']
      });
      ...
      const sedRes = await timedRequest('Next.js', 'exec', 'POST', `/sandbox/${nextSandboxId}/exec`, {
        cmd: ['sed', '-i', 's/"dev": "next dev"/"dev": "next dev -H 0.0.0.0"/', '/home/moulin/workspace/my-app/package.json']
      });
      console.log('sed Response:', sedRes.data);

      // Add custom test content
      const pageContent = `export default function Home() { return <div>Hello Next.js moulin!</div>; }`;
      await timedRequest('Next.js', 'file_write', 'POST', `/sandbox/${nextSandboxId}/file`, {
        path: '/home/moulin/workspace/my-app/app/page.js',
        content: pageContent
      });
      ...
      const devResult = await streamExec(nextSandboxId,
        ['sh', '-c', 'cd /home/moulin/workspace/my-app && npm run dev'],
        {
          waitForText: 'Ready',
          timeoutMs: 120000,
      ...
      assert(proxyContent.includes('Hello Next.js moulin!'), 'Next.js content mismatch via host proxy');
